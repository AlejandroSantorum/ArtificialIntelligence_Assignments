...
(defun num-fichas (tablero)
  (let* ((num 0))
    (loop for col from 0 below (tablero-ancho tablero) do
          (setf num (+ num (altura-columna tablero col))))
    num))

(defun check-sp-st (tablero ficha)
  (if (and (eq(altura-columna tablero 3) (tablero-alto tablero)) (eq(num-fichas tablero) 9)
           (eq(obtener-ficha tablero 4 0) ficha) (eq(obtener-ficha tablero 1 0) ficha)
           (eq(obtener-ficha tablero 5 0) (siguiente-jugador ficha)))
      T
    NIL))
...
(defun check-diag-neg-h (tablero ficha-actual)
  (let ((puntuacion 0))
    ...
    puntuacion))
--------------------------------------------
(defun f-eval-all-lines-upgrade-status (estado)
  ...
  (let* ((tablero (estado-tablero estado))
   ...
   (cond ((not ganador) 0)
      ...
      (let ((puntuacion-actual 0)
        ...
        (if (check-sp-st tablero ficha-actual)
            +val-max+
          (- puntuacion-actual puntuacion-oponente))))))

-----------------------------------------------------
(defun f-eval-all-lines-upgrade (estado)
  ; current player standpoint
  (let* ((tablero (estado-tablero estado))
     (ficha-actual (estado-turno estado))
     (ficha-oponente (siguiente-jugador ficha-actual)))
